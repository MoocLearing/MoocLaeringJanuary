...
namespace Mooc.Web.Areas.Admin.Controllers
{
    ...
    public class CourseController : Controller
    {
        ...
        public JsonResult PageList(int pageIndex, int pageSize)
        {
            ...
            var _course = _dataContext.Courses.Include(c => c.Teacher).Include(c => c.Category);
            var list = _course.OrderByDescending(p => p.ID).Skip(current).Take(pageSize).ToList();
            var courseview = AutoMapper.Mapper.Map<List<CourseView>>(list);
            result.data = courseview;
            result.Count = _course.Count();
        ...
        [HttpPost]
        public JsonResult Save(string CourseName, string CourseDetail, long TeacherId, long CategoryId, int Status)
        {
