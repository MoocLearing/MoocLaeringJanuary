...
namespace Mooc.Web.Controllers
{
    public class PlayController : Controller
    {
        ...
        public ActionResult Index(long id)
        {
            ...
            SubjectController subject = new SubjectController(this._dataContext);
            string msg = string.Empty;
            bool isplay = subject.isPlay(id, out msg, out scheduleId);
            if (!isplay)
                return Content(msg);
            ChapterDto viewmodel = new ChapterDto() { ScheduleId = scheduleId };
            var model = _dataContext.Chapters.Find(id);
